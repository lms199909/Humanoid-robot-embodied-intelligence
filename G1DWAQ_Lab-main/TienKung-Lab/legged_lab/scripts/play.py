...
parser.add_argument("--no_follow_camera", action="store_true",
                    help="Disable viewer camera following the selected robot during play")
...
def play():
    ...
    def update_follow_camera():
        if args_cli.headless or args_cli.no_follow_camera:
            return
        lookat_id = getattr(env, "lookat_id", 0)
        root_pos = env.robot.data.root_pos_w[lookat_id].detach().cpu()
        target = root_pos + torch.tensor([0.0, 0.0, 0.55])
        eye = root_pos + torch.tensor([-3.0, -3.0, 1.45])
